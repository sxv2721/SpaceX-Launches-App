...
describe("<Launch />", () => {
    ...
    describe("renders a", () => {
        const wrapper = getWrapper({ data, isCurrentRoute: true, route: "/" });
        ...
        describe("title and date", () => {
            ...
            it("renders properly", () => {
                ...
                expect(launchDate).toEqual(
                    Moment(data.launch_date_utc).format(
        ...
        describe("description", () => {
            it("opens when description button is clicked", () => {
                expect(wrapper.find(".launchDesc").exists()).toEqual(false);
                wrapper.find(".descButton").simulate("click");
                expect(wrapper.find(".launchDesc").text()).toEqual(
                    data.details
                );
            });

            it("shouldn't render details button without details data", () => {
                const noDetailsWrapper = getWrapper({
                    data: noDetails,
                    isCurrentRoute: true,
                    route: "/",
                });

                expect(noDetailsWrapper.find(".descButton").exists()).toEqual(
                    false
                );
                expect(noDetailsWrapper.find(".launchDesc").exists()).toEqual(
                    false
                );
            });
        });
